...
public class AlloySolution {
	...
	public String toTikz(String conflabel) {
		int i=0;
		int j=0;
		String res ="%id: "+conflabel+"\n\\documentclass{article}\n\\usepackage{tikz}\n";
		res += "\\usetikzlibrary{shadows}\n\\usetikzlibrary{shadows.blur}\n";
		res += "\\begin{document}\n\n";
		res += "\\begin{tikzpicture}[mynode/.style = {draw,inner sep=2mm, rounded corners, fill=black!10, blur shadow={shadow blur steps=5}},scale=4]\n\n";
	    for (AlloySolutionNode node : m_nodes.values()){
	    	String nodeid = node.getId().replace("$", "");
	    	String nodecaption = node.getId().replace("$", "\\_");
	    	
	    	res += "\\node[mynode] ("+nodeid+") at ("+String.valueOf(i)+","+String.valueOf(j)+") {"+nodecaption+"};\n";
	    	if (i<3)
	    		i++;
	    	else {
	    		i=0;
	    ...
	    res+="\\begin{scope}[> = stealth,  ->,blue,thick, every node/.style = {black,right,align=center}]\n";
	    for (AlloySolutionArc arc : m_arcs.values()){
	    	String src = arc.getSource().replace("$", "");
	    	String tgt = arc.getTarget().replace("$", "");
	    	String label = arc.getRelation();
	    	
	    	res += "\\draw ("+src+") edge  node     {"+label+"}     ("+tgt+");\n";
//	    	res += "\\draw ("+src+") edge [bend left]   node     {"+label+"}     ("+tgt+");\n";
	    }
	    res+="\\end{scope}\n\\end{tikzpicture}\n\\end{document}\n";
	...
	public static void main(String[] args){
		...
		ac.generateSolutions("/Users/jcamara/Dropbox/Documents/Work/Projects//GPA.als");
